...
async function loadImage2(link) {
  ...
  try {
    ...
    if (cacheName) {
      ...
      const response = await cache.match(link)
...
function getCacheName(link) {
  ...
  if (link.includes('/shop/')) return CACHE_SHOP
...
async function putCache(link, response) {
  const cacheName = getCacheName(link)
  if (!cacheName) return
  const cache = await caches.open(cacheName)
  const responseClone = response.clone()
  await cache.put(link, responseClone)
}
